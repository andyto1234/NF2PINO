...
_HYBRID_ROOT = Path(__file__).resolve().parents[2].parent
if str(_HYBRID_ROOT) not in sys.path:
    sys.path.insert(0, str(_HYBRID_ROOT))
try:
    from nf2pino.nf2_init import inject_pino_init_config
except ImportError:
    inject_pino_init_config = None
...
def run(base_path, data, work_directory=None, callbacks=[], logging={}, model={}, training={}, config=None):
    ...
    if training.get('init_mode') == 'pino' or training.get('init_from_pino') is not None:
        if inject_pino_init_config is None:
            raise ImportError('PINO initialization requires the nf2pino package to be importable')
        training = inject_pino_init_config(training, model_kwargs=model, data_config=data_module.config,
                                           base_path=base_path)
    ...
    trainer_devices = training.get('devices', n_gpus if n_gpus > 0 else (1 if has_mps else None))
    trainer_accelerator = training.get('accelerator', 'gpu' if n_gpus >= 1 else ('mps' if has_mps else None))
    if trainer_accelerator == 'mps' and trainer_devices is None:
        trainer_devices = 1

    trainer_strategy = 'dp' if n_gpus > 1 and trainer_accelerator == 'gpu' else None
    ...
    # Lightning 2.x: strategy=None is invalid; omit for single-device / data-parallel default.
    if trainer_strategy is not None:
        trainer_kwargs['strategy'] = trainer_strategy

    trainer = Trainer(**trainer_kwargs)

    last_ckpt = os.path.join(base_path, 'last.ckpt')
    ckpt_arg = last_ckpt if os.path.isfile(last_ckpt) else None
    trainer.fit(nf2, data_module, ckpt_path=ckpt_arg)
