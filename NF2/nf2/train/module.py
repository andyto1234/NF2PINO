...
class NF2Module(LightningModule):
    ...
    def __init__(self, validation_mapping, data_config, model_kwargs=None, loss_config=None,
        ...
        if meta_path:
            ...
            if meta_path.endswith('nf2'):
                state_dict = meta_state['model'].state_dict()
            elif 'm' in meta_state:
                state_dict = meta_state['m']
            else:
                raise KeyError(f"Meta checkpoint {meta_path} must contain key 'm' or be an .nf2 artifact")
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx) -> None:
        ...
        # PyTorch Lightning 2.x: cannot pass a dict as the value to self.log; use log_dict.
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx):
        ...
        out = {k: v.detach() for k, v in {**batch, **model_out}.items()}
        self._val_step_outputs_by_idx.setdefault(dataloader_idx, []).append(out)
        return out

    def on_validation_epoch_end(self):
        """Aggregate validation batches (replaces removed ``validation_epoch_end`` in Lightning 2.x)."""
        by_idx = getattr(self, '_val_step_outputs_by_idx', None) or {}
        self.validation_outputs = {}
        if not by_idx:
            return
        sorted_idx = sorted(by_idx.keys())
        outputs_list = [by_idx[i] for i in sorted_idx]
        if len(outputs_list) == 0 or any(len(o) == 0 for o in outputs_list):
            return

        for i, outputs in enumerate(outputs_list):
            out_keys = outputs[0].keys()
            stacked = {k: torch.cat([o[k] for o in outputs]) for k in out_keys}
            self.validation_outputs[self.validation_mapping[i]] = stacked
