...
def get_potential(b_n, height, batch_size=2048, strides=(1, 1, 1), progress=True, num_workers=0):
    ...
    with torch.no_grad():
        ...
        potential = []
        loader = DataLoader(TensorDataset(coords), batch_size=batch_size, num_workers=num_workers)
...
def _compute_fields(coords, cube_shape, b_n, batch_size=2048, progress=False, num_workers=0):
    ...
    with torch.no_grad():
        ...
        potential = []
        iter = DataLoader(TensorDataset(flat_coords), batch_size=batch_size, num_workers=num_workers)
