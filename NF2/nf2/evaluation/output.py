...
class BaseOutput:
    ...
    def load_coords(self, coords, batch_size=int(2 ** 12), progress=False, compute_jacobian=True, metrics=None):
        ...
        g_scale = float(self.state['data']['G_per_dB'])
        m_per_ds = float(self.state['data']['Mm_per_ds']) * 1e6  # meters per ds
        ...
        def _load(coords):
            ...
            # Numeric scale (avoid Quantity × ndarray edge cases across astropy versions).
            model_out['b'] = model_out['b'] * g_scale
            if 'a' in model_out:
                model_out['a'] = model_out['a'] * g_scale * m_per_ds
            if 'jac_matrix' in model_out:
                model_out['jac_matrix'] = model_out['jac_matrix'] * g_scale / m_per_ds
            return model_out
...
class CartesianOutput(BaseOutput):
    ...
    def load_slice(self, z=0 * u.Mm, Mm_per_pixel=None, **kwargs):
        ...
        z_mm = float(z.to_value(u.Mm))
        z_ds_coord = z_mm / float(self.Mm_per_ds)
        coords = np.stack(
            np.meshgrid(np.linspace(x_min, x_max, np.round((x_max - x_min) / self.ds_per_pixel + 1).astype(int)),
                        ...
                        np.ones((1,), dtype=np.float32) * z_ds_coord, indexing='ij'), -1)
        coords = coords[:, :, 0]
